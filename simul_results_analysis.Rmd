...
temp_colnames <- c("method","delta_beta","noise","rep","TP_bp",
                   "TP_feature_called_prop", ## 1 - this = FP proportion
                   "TP_feature_simul_prop",  ## 1 - this = FN proportion
                   "FP_bp","FN_bp","AUPR","MCC","F1","Time")
OPERATING_STATS <- data.frame(matrix(NA, nrow=length(simul_results),
                                     ncol=length(temp_colnames)), )
colnames(OPERATING_STATS) <- temp_colnames
...
for(i in 1:length(simul_results)){
  ...
  OPERATING_STATS[i,]$method <-names(method_set_list)[method_id]
  OPERATING_STATS[i,]$delta_beta <- simul$pars$delta_beta
  OPERATING_STATS[i,]$noise <- simul$pars$noise
  OPERATING_STATS[i,]$rep <- simul$pars$rep
  ...
  cs_intersect <-  GenomicRanges::intersect( called_grs, simul_grs)
  OPERATING_STATS[i,]$TP_bp <- sum(cs_intersect[which]@ranges@width)
  
  
  OPERATING_STATS[i,]$TP_feature_called_prop <- -1
  
  
  OPERATING_STATS[i,]$TP_feature_simul_prop <- -1
  
  
  OPERATING_STATS[i,]$FP_bp <- (sum(simul_grs@ranges@width) -
                                  OPERATING_STATS[i,]$TP_bp)
  
  OPERATING_STATS[i,]$FN_bp <- -1
  
  
  OPERATING_STATS[i,]$AUPR <- -1
  
  
  OPERATING_STATS[i,]$MCC <- -1
  
  
  OPERATING_STATS[i,]$F1 <- -1
  
  
  OPERATING_STATS[i,]$TIME <- -1
