...
write.csv(WIDTH_PAIRS, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"WIDTH_PAIRS.csv",sep = ""))
write.csv(MAPPING_VALUES, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"MAPPING_VALUES.csv",sep=""))
write.csv(CALLED_TP_FP, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"CALLED_TP_FP.csv",sep=""))
write.csv(TIME, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"TIME.csv",sep=""))
save.image( paste(format(Sys.time(),"%Y-%m-%d_"),"Post_simul.Rdata",sep = ""))
```
...
gg<- ggplot(TIME, aes(x = paste(method,"\n",params) , y = log10(time)))+
...
filename<-paste(results_dir, date, "_",  "methods_time_flat.png", sep = "")

ggsave(
  filename=filename, plot = gg, device = "png", path = NULL, scale = 1,
  width = 6, height = 3, units = c("in"), dpi = 320
)
