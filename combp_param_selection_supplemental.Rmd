...
gg <- ggplot(WIDTH_PAIRS, aes(x=log10(simulated_width), y=log10(called_width))) +
    ...
    facet_wrap( . ~ params, labeller=labeller(params = labels), ncol=1)
  gg
  
  filename <- paste("./results_2/",format(Sys.time(),"%Y-%m-%d"), "combp_param_test_simul_v_call_width" , ".png",sep="")
  ...
  ggsave( filename=filename, plot = gg, device = "png", path = NULL,
    scale = 1, width = 2, height = 10, units = c("in"), dpi = 320)
...
gg <- ggplot(MAPPING_VALUES, aes(x=log10(mapping_value))) +
  ...
  facet_wrap( . ~ params, ncol=1 )#, labeller=labeller(method = labels) )
gg

filename <- paste("./results_2/",format(Sys.time(),"%Y-%m-%d"), "combp_params_mapping_values" , ".png",sep="")
#tiff(filename = filename, width = 8, height=4, units = "in", res = 500, pointsize = 1)
#grid.arrange(gg, ncol=1)
#dev.off()
ggsave( filename=filename, plot = gg, device = "png", path = NULL,
