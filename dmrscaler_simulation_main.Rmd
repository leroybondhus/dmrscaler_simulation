...
rownames(simul_pars) <- NULL

simul_constructor_list <- list()

for(simul_pars_id in 1:nrow(simul_pars)){
  ## identify CGs associated with simulated DMR
    ...
    while(count > 0){
        ...
        if(length(x_which) >= min_cgs  ## at least min_cgs 
           & sum(locs$in_dmr[x_which_left:x_which_right])==0   ## not overlapping a dmr that already exists
            ...
            x_which <- unique(c(min(x_which), sample(x_which, ceiling((1-simul_pars$noise[simul_pars_id]) * length(x_which) )  ) ,max(x_which))) ## sample left and right most cg and introduce noise 
  ...
  simul_constructor_list_el <- paste(colnames(simul_pars),simul_pars[simul_pars_id,], sep = "__", collapse = "___")
  simul_constructor_list[[simul_constructor_list_el]]$dmr_locs <- locs[which(locs$in_dmr == 1),]
  
  ## identify g1 and g2 
  g12 <- sample(colnames(B), 2*simul_pars$num_samples[simul_pars_id], replace = FALSE)
  g1 <- g12[1:simul_pars$num_samples[simul_pars_id]]
  g2 <- g12[(simul_pars$num_samples[simul_pars_id]+1):length(g12)]
  simul_constructor_list[[simul_constructor_list_el]]$g1 <- g1
  simul_constructor_list[[simul_constructor_list_el]]$g2 <- g2
