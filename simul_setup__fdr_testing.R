...
results_dir <-paste("./results/fdr_testing/")
...
method_set_list <- list(
  dmrscaler_fdr_01 = list(method="dmrscaler",
  ...
  dmrscaler_fdr_05 = list(method="dmrscaler",
                     function_call=" DMRscaler::dmrscaler(locs = locs, locs_pval_cutoff = 0.01, region_signif_method = \"ben\", region_signif_cutoff = 0.05, window_type = \"k_near\", window_sizes = c(2,4,8,16,32,64), output_type = \"comp\") " ),
  dmrscaler_fdr_10 = list(method="dmrscaler",
                     function_call=" DMRscaler::dmrscaler(locs = locs, locs_pval_cutoff = 0.01, region_signif_method = \"ben\", region_signif_cutoff = 0.10, window_type = \"k_near\", window_sizes = c(2,4,8,16,32,64), output_type = \"comp\") " ),
  dmrscaler_fdr_15 = list(method="dmrscaler",
                     function_call=" DMRscaler::dmrscaler(locs = locs, locs_pval_cutoff = 0.01, region_signif_method = \"ben\", region_signif_cutoff = 0.15, window_type = \"k_near\", window_sizes = c(2,4,8,16,32,64), output_type = \"comp\") " ),
  dmrscaler_fdr_20 = list(method="dmrscaler",
                     function_call=" DMRscaler::dmrscaler(locs = locs, locs_pval_cutoff = 0.01, region_signif_method = \"ben\", region_signif_cutoff = 0.20, window_type = \"k_near\", window_sizes = c(2,4,8,16,32,64), output_type = \"comp\") " )

)


## write Rdata objects to load into each simul_individual_run.R script
filename <- paste("fdr_simul_setup.Rdata")
save.image(file=filename)
## write simul_table
simul_table <- names(simul_constructor_list)
filename <- paste("fdr_simul_table.csv")
write.table(simul_table, filename, row.names = F, col.names=F, sep=",")

## write method_table
method_table <- names(method_set_list)
filename <- paste("fdr_method_table.csv")
