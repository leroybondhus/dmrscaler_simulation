...
for(RUN in 1:10){
        ...
        TP <- intersect( called_grs, simul_grs)##### write this....
        ### save count_called_per_simul vector. To plot : density of log10 values.
...
CALLED_TP_FP <- data.frame("method"=character(), "params"=character(),
        ...
        for(i in 1:length(simul_grs)){
          ...
          which <- dmrcate_overlaps@to[which(dmrcate_overlaps@from==i)]  ## select rows of overlaps corresponding to simulated dmr
          max_overlapping_those_called_over_sim <-max(overlap_count_called_df[which,"overlap_counts"])
          if(overlaps_count_simul[i] >= max_overlapping_those_called_over_sim){ count_called_per_simul[i] <- overlaps_count_simul[i]}
          else{count_called_per_simul[i] <- 1/max_overlapping_those_called_over_sim}  ## fraction if multiple simulated map to single called
        }
        ...
        BH_MAPPING_VALUES <- rbind(BH_MAPPING_VALUES, data.frame("mapping_value"= unname(count_called_per_simul)))
        ...
        dmrcate_results <- extractRanges(dmrcoutput = dmrcate_results, genome="hg19")
        dmrcate_overlaps <- findOverlaps(simul_grs, dmrcate_results)
        DMRCATE_WIDTH_PAIRS <- rbind(DMRCATE_WIDTH_PAIRS, data.frame("simulated_width"=simul_grs@ranges@width[dmrcate_overlaps@from], "called_width"= dmrcate_results@ranges@width[dmrcate_overlaps@to], "param_string"="lambda=1e6_C=2000" ))
        
        overlaps_count_simul <- countOverlaps( simul_grs, dmrcate_results)
        overlaps_count_simul 
        
        overlaps_count_called <- countOverlaps( dmrcate_results, simul_grs)
        overlaps_count_called
        overlap_count_called_df <- data.frame("names"=as.numeric(names(overlaps_count_called)), "overlap_counts" = overlaps_count_called) 
        ...
        for(i in 1:length(simul_grs)){
          print(i)
          if(!is.element(i, dmrcate_overlaps@from)){ count_called_per_simul[i] <- 0; next;}
        ...
        DMRCATE_MAPPING_VALUES <- rbind(DMRCATE_MAPPING_VALUES, data.frame("mapping_value"= unname(count_called_per_simul), "param_string"="lambda=1e6_C=2000"))
        ...
        dmrcate_results_default <- dmrcate(myannotation, lambda=1e3, C=2)
        dmrcate_results_default <- extractRanges(dmrcoutput = dmrcate_results_default, genome="hg19")
        
        dmrcate_overlaps <- findOverlaps(simul_grs, dmrcate_results_default)
        DMRCATE_WIDTH_PAIRS <- rbind(DMRCATE_WIDTH_PAIRS, data.frame("simulated_width"=simul_grs@ranges@width[dmrcate_overlaps@from], "called_width"= dmrcate_results_1e6@ranges@width[dmrcate_overlaps@to], "param_string"="Default_lambda=1000_C=2" ) )
