...
results_dir <-paste(getwd(), "/results_2/",sep = "")
```
...
for(RUN in 1:3){
        ...
        pars <- data.frame("delta_beta" = 0.4, "num_samples" = 8, "noise" = 0.5, "dmr_buffer" = 10) 
        ...
        ## {r}
            ...
            print(paste("On to dmrscaler RUN IS :",RUN))
              ...
              t1 <- Sys.time() 
              dmrscaler_result <- dmrscaler::dmrscaler(locs = locs, fdrscaler = fdrscaler, cltable=cltable, layer_sizes = layer_sizes)
      ...
      #  plot(log10(simul_grs@ranges@width[overlaps@from]), log10(called_grs@ranges@width[overlaps@to]))
        ...
        write.csv(WIDTH_PAIRS, file = paste("./results/",format(Sys.time(),"%Y-%m-%d-%H%M%S_"),"WIDTH_PAIRS.csv",sep=""))
        write.csv(MAPPING_VALUES, file = paste("./results/",format(Sys.time(),"%Y-%m-%d-%H%M%S_"),"MAPPING_VALUES.csv",sep=""))
        write.csv(CALLED_TP_FP, file = paste("./results/",format(Sys.time(),"%Y-%m-%d-%H%M%S_"),"CALLED_TP_FP.csv",sep=""))
        write.csv(TIME, file = paste("./results/",format(Sys.time(),"%Y-%m-%d-%H%M%S_"),"TIME.csv",sep=""))
...
write.csv(WIDTH_PAIRS, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"_dB0.4_WIDTH_PAIRS.csv",sep = ""))
write.csv(MAPPING_VALUES, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"_dB0.4_MAPPING_VALUES.csv",sep=""))
write.csv(CALLED_TP_FP, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"_dB0.4_CALLED_TP_FP.csv",sep=""))
write.csv(TIME, file = paste("./results/",format(Sys.time(),"%Y-%m-%d_"),"_dB0.4_TIME.csv",sep=""))
...
filename<-paste(results_dir, date, "_ggsave_",  "sim_v_called_width_vert.png", sep = "")
...
filename<-paste(results_dir, date, "_",  "mapping_ratios_vert.png", sep = "")
#tiff(filename = filename, width = 8, height=4, units = "in", res = 500, pointsize = 1)
...
gg <- ggplot(df_temp, aes(x=R_bp, y=P_bp))+
  ...
  labs(subtitle = paste(
         ...
         ", Comb-p AUC=", round(auc[["combp"]]$bp_auc,3)
         ))+
  xlab("Recall")+
  ylab("Precision")+
...
filename<-paste(results_dir, date, "_",  "AUCPR_bp.png", sep = "")
#tiff(filename = filename, width = 8, height=4, units = "in", res = 500, pointsize = 1)
