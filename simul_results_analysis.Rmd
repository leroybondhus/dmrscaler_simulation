...
for(i in 1:length(GLOBAL_OPERATING_STATS)){
  ...
  temp <- GLOBAL_OPERATING_STATS[[i]]
  temp_df <-aggregate(cbind(Precision, Recall, Specificity, FDR, F1, MCC, CG_PROP_DIFF ) ~ method + delta_beta + noise, data=temp, FUN=mean, na.rm=TRUE)
  GOS_summary[[i]]$gos_means <- temp_df
  temp_df <-aggregate(cbind(Precision, Recall, Specificity, FDR, F1, MCC, CG_PROP_DIFF) ~ method + delta_beta + noise, data=temp, FUN=sd, na.rm=TRUE)
