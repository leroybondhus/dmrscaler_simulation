...
gg <- ggplot(WIDTH_PAIRS, aes(x=log10(simulated_width), y=log10(called_width))) +
    ...
    facet_grid( . ~ params, labeller=labeller(params = labels))
  gg
  
  filename <- paste("./results/",format(Sys.time(),"%Y-%m-%d"), "combp_params_simul_v_call_width" , ".png",sep="")
  ...
  ggsave( filename=filename, plot = gg, device = "png", path = NULL,
    scale = 1, width = 8, height = 2, units = c("in"), dpi = 320)
...
gg <- ggplot(MAPPING_VALUES, aes(x=log10(mapping_value))) +
  ...
  facet_grid(. ~ params )#, labeller=labeller(method = labels) )
gg

filename <- paste("./results/",format(Sys.time(),"%Y-%m-%d"), "combp_params_mapping_values" , ".png",sep="")
#tiff(filename = filename, width = 8, height=4, units = "in", res = 500, pointsize = 1)
#grid.arrange(gg, ncol=1)
#dev.off()
ggsave( filename=filename, plot = gg, device = "png", path = NULL,
