...
for(i in 1:length(simul_results)){
  ...
  OPERATING_STATS$basepair[i,]$P <- sum(simul_grs@ranges@width) 
  OPERATING_STATS$basepair[i,]$N <- sum(simul_inverse_grs@ranges@width)
  OPERATING_STATS$basepair[i,]$TP_called <- sum(called_grs$TP * called_grs@ranges@width)
  OPERATING_STATS$basepair[i,]$TP_simul <- sum(simul_grs$TP * simul_grs@ranges@width)
  OPERATING_STATS$basepair[i,]$TN <- sum(simul_inverse_grs@ranges@width) - sum(cs_inverse_intersect@ranges@width)
  ## end: basepair operating characteristics
  ## start: cpg_probe operating characteristics
  OPERATING_STATS$cpg_probe[i,]$P <- sum( GenomicRanges::countOverlaps(simul_grs, locs_gr) ) 
  OPERATING_STATS$cpg_probe[i,]$N <- sum( GenomicRanges::countOverlaps(simul_inverse_grs, locs_gr) )
  OPERATING_STATS$cpg_probe[i,]$TP_called <- length(GenomicRanges::findOverlaps(locs_gr, cs_intersect)@from)
  OPERATING_STATS$cpg_probe[i,]$TP_simul <- length(GenomicRanges::findOverlaps(locs_gr, cs_intersect)@from)
  temp <- length(GenomicRanges::findOverlaps(locs_gr, cs_inverse_intersect)@from)  
  OPERATING_STATS$cpg_probe[i,]$TN <- (OPERATING_STATS$cpg_probe[i,]$N - 
