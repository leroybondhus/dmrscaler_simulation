...
for(method_id in 1:length(unique(MAPPING_VALUES$method))){
  ...
  ggplot(dat, aes(x=x,y=cumsum))+geom_line()+facet_grid(delta_beta ~ noise)
  ...
  gg <- ggplot(MAPPING_VALUES[which,], aes(x=log10(mapping_value)))+
                 ...
                 theme(
                   ...
                   strip.text = element_text(face="bold", size=10, angle=0),
...
dat$cumsum <- numeric(length=nrow(dat))
dat$delta_beta <- temp$layout$layout$delta_beta[which(temp$layout$layout$PANEL==p)]
dat$noise <- temp$layout$layout$noise[which(temp$layout$layout$PANEL==p)]
for(p in unique(dat$PANEL) ){
  dat$cumsum[which(dat$PANEL==p)] <- cumsum(dat$y[which(dat$PANEL==p)])
  dat <- rbind(dat, dat[which(dat$PANEL==p),])
}
ggplot(dat, aes(x=x, y=cumsum))+
  geom_line()+facet_wrap( delta_beta ~ noise)
...
#  stat_bin(aes(y=cumsum(stat(count)/ simul_count) ),geom="line",color="green")+
  ...
  scale_x_continuous(breaks = seq(-1,max(log10(MAPPING_VALUES$mapping_value)),1))+
  xlab("-log10(Mapping value) ")+
  theme_bw()+
  theme(
    plot.background = element_blank(),
  ...
  facet_wrap(method ~ . , ncol = 4 )# , labeller=labeller(method = labels) )
gg

filename<-paste(results_dir, date, "_",  "ashg_poster_mapping_ratios_vert.png", sep = "")
