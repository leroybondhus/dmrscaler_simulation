...
for(i in 1:length(GGPLOT_LIST$ggl_width_pairs)){
  ...
  ggsave(
    ...
    path = NULL,
    scale = 1,
    width = ifelse(grepl("partition_by_method",filename), 3,6),
    height = 6,
    units = c("in"),
...
for(i in 1:length(os_summary)){
  ...
  title <- paste("p_r_partition_by_method__",names(os_summary)[i] , sep="")
...
for(i in 1:length(GGPLOT_LIST$P_R)){
  ...
  ggsave(
    ...
    path = NULL,
    scale = 1,
    width = ifelse(grepl("partition_by_method",filename), 5,8),
    height = ifelse(grepl("partition_by_method",filename), 3,7),
    units = c("in"),
    dpi = ifelse(grepl("partition_by_method",filename), 320,230)
  )
}
...
for(i in 1:length(GOS_summary)){
  ...
  write.csv(GOS_summary[[i]]$gos_mean_std, file = filename)
  filename <- paste(tables_dir, "global_operating_stats_summary_2only__", names(GOS_summary)[i],".csv", sep="")
  write.csv(GOS_summary[[i]]$gos_mean_std[grep("_2",GOS_summary[[i]]$gos_mean_std$method),], file = filename)
}


for(i in 1:length(aucpr_list)){
  filename <- paste(tables_dir, "aucpr_", names(aucpr_list)[i],".csv", sep="")
  write.csv(aucpr_list[[i]]$aucpr_df, file = filename)
  filename <- paste(tables_dir, "aucpr_2_only_", names(GOS_summary)[i],".csv", sep="")
  write.csv(aucpr_list[[i]]$aucpr_df[grep("_2",aucpr_list[[i]]$aucpr_df$method),], file = filename)
}
