...
install("/home/leroy/Desktop/PROJECTS/dmrscaler", quiet=T)
...
for(RUN in 1:5){
        ...
        ## {r}
            ...
            #  data$chr <- droplevels(data$chr)
              ...
              TIME <- rbind(TIME, data.frame("method" = "dmrscaler",
                               "params" = "default",
                               "time"=as.numeric(difftime(t1,t2, "secs")) ) )
        ...
        t1 <- Sys.time()
        system("comb-p pipeline -c 4 --dist 1000000 --step 1000 --seed 1e-3 -p ./intermediate_data_2/test --region-filter-p 0.1 ./intermediate_data_2/combp_temp_input.bed ")
        t2 <- Sys.time()
        TIME <- rbind(TIME, data.frame("method" = "combp",
                           "params" = "dist=1e6_step=1e3",
                           "time"=as.numeric(difftime(t1,t2, "secs")) ) )
