...
install("/home/leroy/Desktop/PROJECTS/dmrscaler")
...
for(RUN in 1:1){
        ...
        Sys.time()
        system("comb-p pipeline -c 4 --dist 100000 --step 5000 --seed 1e-3 -p ./intermediate_data_2/test100k5k --region-filter-p 0.1 ./intermediate_data_2/combp_temp_input.bed ")
        Sys.time()
        

        ## Parse comb-p resuults into the CALLED_TP_FP and the MAPPING VALUES tables
        combp_results <- data.table::fread("./intermediate_data_2/test100k5k.regions-t.bed")
        ...
        write.csv(WIDTH_PAIRS, file = paste("./results/",format(Sys.time(),"%Y-%m-%d"),"WIDTH_PAIRS.csv"),sep = "")
        write.csv(MAPPING_VALUES, file = paste("./results/",format(Sys.time(),"%Y-%m-%d"),"MAPPING_VALUES.csv"),sep="")
        write.csv(CALLED_TP_FP, file = paste("./results/",format(Sys.time(),"%Y-%m-%d"),"CALLED_TP_FP.csv"),sep="")
