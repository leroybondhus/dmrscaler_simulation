...
gg <- ggplot(WIDTH_PAIRS, aes(x=log10(simulated_width), y=log10(called_width))) +
  ...
  filename <- paste("./results/",format(Sys.time(),"%Y-%m-%d"), "combp_params_simul_v_call_width" , ".tiff",sep="")
  #tiff(filename = filename, width = 8, height=4, units = "in", res = 500, pointsize = 1)
  #grid.arrange(gg, ncol=1)
  #dev.off()
  ggsave( filename=filename, plot = gg, device = "tiff", path = NULL,
    scale = 1, width = 6, height = 2.5, units = c("in"), dpi = 320)
