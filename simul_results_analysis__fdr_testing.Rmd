...
if(file.exists("fdr_testing_GLOBAL_OPERATING_STATS.Rdata")){
  ...
  GLOBAL_OPERATING_STATS <- foreach(i = 1:length(simul_results),
    ...
    temp_colnames <- c("method","delta_beta","noise","rep",
                       "cg_order_rand", "cg_fdr_cutoff","region_pval_cutoff",
    ...
    for(j in 1:length(OP_STATS)){
      ...
      OP_STATS[[j]]$region_pval_cutoff <- simul$pars$region_pval_cutoff
    }
...
for(i in 1:length(GLOBAL_OPERATING_STATS)){
  ...
  temp_df <-aggregate(cbind(Precision, Recall, Specificity, FDR, F1, MCC, CG_PROP_DIFF ) ~ method + delta_beta + noise + 
                        cg_order_rand + cg_fdr_cutoff + region_pval_cutoff, data=temp, FUN=mean, na.rm=TRUE)
  GOS_summary[[i]]$gos_means <- temp_df
  temp_df <-aggregate(cbind(Precision, Recall, Specificity, FDR, F1, MCC, CG_PROP_DIFF) ~ method + delta_beta + noise + 
                        cg_order_rand + cg_fdr_cutoff + region_pval_cutoff, data=temp, FUN=sd, na.rm=TRUE)
