...
```{r create timing plots}
filenames <- list.files(path = "./results/simuls/", pattern = "*TIME.csv", full.names = T)

simul_times <- data.frame("method"=character(),"simul_id"=numeric(),"time"=numeric())
...
for(simul_id in 1:length(simul_constructor_list)){
  for(method_id in 1:length(method_set_list)){
    filename <- filenames[grep(paste("simul_set_",simul_id,
                                     ...
                                     sep=""), filenames)]
    temp <- fread(filename)
    simul_times <- rbind(simul_times,
                         data.frame(method=names(method_set_list)[method_id],
                                    simul_id=simul_id,
                                    time=temp$time))
  }
}

simul_times$method <- factor(simul_times$method, levels = rev(levels(as.factor(simul_times$method) )))

gg <- ggplot(simul_times[grep("_2",simul_times$method),], aes(y=method, x=log10(time)))+
  ggtitle("method runtimes")+
  xlab("runtime: log10(seconds)")+
  geom_point()+
  geom_point()+
  geom_boxplot()+
  theme_bw()+
...
# filename <- paste(figures_dir, "method_runtimes.png",sep = "") 
# ggsave(
#   filename=filename,
#   plot = gg,
#   device = "png",
...
if(file.exists("simul_GLOBAL_OPERATING_STATS.Rdata")){
  load("simul_GLOBAL_OPERATING_STATS.Rdata")
} else{
...
if(file.exists("simul_OPERATING_STATS.Rdata")){
  load("simul_OPERATING_STATS.Rdata")
} else{
  ...
  save(OPERATING_STATS, file="simul_OPERATING_STATS.Rdata")
}
...
for(i in 1:length(GLOBAL_OPERATING_STATS)){
  ...
  export_gos$method <- gsub("bumphunter_2", "bumphunter:maxWidth=1Mb",export_gos$method)
  export_gos$method <- gsub("combp_2", "comb-p:dist=1Mb,step=5kb",export_gos$method)
  export_gos$method <- gsub("dmrcate_2", "DMRcate:lambda=1Mb,C=2000",export_gos$method)
  export_gos$method <- gsub("dmrscaler_2", "DMRscaler:benj-yuk",export_gos$method)
  export_gos$method <- gsub("bumphunter_1", "bumphunter:maxWidth=1Mb",export_gos$method)
  export_gos$method <- gsub("combp_1", "comb-p:dist=1Mb,step=5kb",export_gos$method)
  export_gos$method <- gsub("dmrcate_1", "DMRcate:lambda=1Mb,C=2000",export_gos$method)
  export_gos$method <- gsub("dmrscaler_1", "DMRscaler:bonferroni",export_gos$method)
  filename <- paste(tables_dir, "global_operating_stats_", names(GLOBAL_OPERATING_STATS)[i],".csv", sep="")
...
for(i in 1:length(GLOBAL_OPERATING_STATS)){
  ...
  temp <- GLOBAL_OPERATING_STATS[[i]]
  temp <- merge(temp, aucpr_list[[i]]$aucpr_df, sort=F)
  temp$AUCPR <- paste(as.character(temp$aucpr),"(\u00b1",
                                    as.character(signif(temp$aucpr_sd,1)), ")", sep="")
  temp$aucpr <- NULL
  temp$aucpr_sd <- NULL
  
  
  temp_df <-aggregate(cbind(Precision, Recall, Specificity, FDR, F1, MCC, CG_PROP_DIFF) ~ method + delta_beta + noise + AUCPR, data=temp, FUN=mean, na.rm=TRUE)
  GOS_summary[[i]]$gos_means <- temp_df
  temp_df <-aggregate(cbind(Precision, Recall, Specificity, FDR, F1, MCC, CG_PROP_DIFF) ~ method + delta_beta + noise + AUCPR, data=temp, FUN=sd, na.rm=TRUE)
  GOS_summary[[i]]$gos_std <- temp_df
  
  temp_df <- GOS_summary[[i]]$gos_means
  temp_df[,5:ncol(temp_df)] <- signif(temp_df[,5:ncol(temp_df)], digits=2)
  temp_df[,5:ncol(temp_df)] <- apply(temp_df[,5:ncol(temp_df)], 2, as.character)
  
  GOS_summary[[i]]$gos_mean_std <- temp_df
  
  temp_df <- GOS_summary[[i]]$gos_std
  temp_df[,5:ncol(temp_df)] <- signif(temp_df[,5:ncol(temp_df)], digits=1)
  temp_df[,5:ncol(temp_df)] <- apply(temp_df[,5:ncol(temp_df)], 2, as.character)
  
  temp_df[,5:ncol(temp_df)] <- paste(as.matrix(GOS_summary[[i]]$gos_mean_std[,5:ncol(temp_df)]),"(\u00b1", as.matrix(temp_df[,5:ncol(temp_df)]),")", sep=""  )
  GOS_summary[[i]]$gos_mean_std <- temp_df
}
