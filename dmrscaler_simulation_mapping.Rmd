...
MAPPING_VALUES <- read.csv(file = "./results/10232020_MAPPING_VALUES.csv")
...
gg <- ggplot(MAPPING_VALUES[which(MAPPING_VALUES$simul_width<1e4),], aes(x=log10(mapping_value))) +
  #ggtitle("simulated v called dmr length log10")+
  #xlab("log10( Simulated DMR Width )")+
  #ylab("log10( Called DMR Width )")+
  #stat_density_2d(aes(fill= ..level..),geom = "polygon",  n=50, h=0.7, alpha=0.5) +
...
for(i in 1:length(P_R)){
  df_temp <- rbind(df_temp, cbind(P_R[[i]],"method"=rep(names(P_R)[i]))) 
}
