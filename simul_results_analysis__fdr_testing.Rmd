...
if(file.exists("fdr_testing_GLOBAL_OPERATING_STATS.Rdata")){
  load("fdr_testing_GLOBAL_OPERATING_STATS.Rdata")
} else{
  ...
  GLOBAL_OPERATING_STATS <- foreach(i = 1:length(simul_results),
    ...
    temp_colnames <- c("method","delta_beta","noise","rep",
                       "cg_order_rand", "cg_fdr_cutoff","region_fdr_cutoff",
    ...
    for(j in 1:length(OP_STATS)){
      ...
      OP_STATS[[j]]$region_fdr_cutoff <- simul$pars$region_fdr_cutoff
    }
  ...
  save(GLOBAL_OPERATING_STATS, file="fdr_testing_GLOBAL_OPERATING_STATS.Rdata")
}
...
if(file.exists("fdr_testing__OPERATING_STATS.Rdata")){
  ...
  OPERATING_STATS <- foreach(i = 1:length(simul_results),
      ...
      temp_seq <- floor(seq(from=1,to=length(called_grs),
                            length.out = min(length(called_grs),5)))
      temp_colnames <- c("method","delta_beta","noise","rep",
